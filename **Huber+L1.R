...
for (t in (1:T)){
  ...
  for (l in (1:20)){
    ...
    for (j in (1:p)){
      ...
      else if(abs(a)>gamma*lambda02){
        bet[j]<-b
      }
      else{
        bet[j]<-sign(b)*(abs(b)-gamma*lambda02/((gamma-1)*a)/(1-1/((gama-1)*a)))
      }
    }
  }
    ...
    for (kk2 in lambda1){
      for (kk3 in lambda2){
        ...
        while(((l>=1)&&(l<=L))&&((norm(alp-alpp)/sqrt(n)+norm(alp-alpp)/sqrt(p))>sigma)){
          ...
          #更新alpha
          for (i in (1:n)){
            a<-y[i]-x[i,]%*%bet-s[i]+v[i]/rho-mu[i]
            if(a+kk2/rho<0){
              alp[i]<-y[i]-x[i,]%*%bet-s[i]+v[i]/rho+kk2/rho
            }
            else if(a-kk2/rho>0){
              alp[i]<-y[i]-x[i,]%*%bet-s[i]+v[i]/rho-kk2/rho
            }
          ...
          for (j in (1:p)){
            ...
            else if(abs(a)>gamma*kk3){
              bet[j]<-b
            }
            else{
              bet[j]<-sign(b)*(abs(b)-gamma*kk3/((gamma-1)*a)/(1-1/((gama-1)*a)))
            }
          }
          #更新s
          for (i in (1:n)){
        ...
        bet[bet<0.005]<-0
        qq<-sum(abs(bet)>0)
        bic<-log(sum((y-alp-x%*%bet)^2/2*(abs(y-alp-x%*%bet)<=tau)+(tau*abs(y-alp-x%*%bet)-tau^2/2)*(abs(y-alp-x%*%bet)>tau))/n)+2*log(n*kk1+p)*(qq+kk1)*log(n)/n                              
  ...
  print(t)
  end_time <- Sys.time()
  TIMES[t]<-as.numeric(end_time - start_time)
}
{
